...
player_labels={}
cluster_lines = [[], [], []] # one list for each cluster
prev_line_update = 0
player_lines={}
def animate(i):
    ...
    """ WORKS
    # Create lines connecting players in the same cluster
    if i % 25 == 0:
        for c in range(3):
            ...
            for line in cluster_lines[c]:
                line.remove()
            line = ax.plot(x, y, linewidth=2, color='white')
            cluster_lines[c] = line
            """
    if i % 5 == 0:
        for c in range(3):
            cluster_frame = home_frame[home_frame['cluster'] == c]
    ...
    for i, row in home_frame.iterrows(): 
        x, y = row['x'], row['y']
